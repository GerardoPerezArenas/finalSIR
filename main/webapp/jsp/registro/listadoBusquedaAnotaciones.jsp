    ...
    <head>
        ...
        function cargaPagina(numeroPagina){
            ...
            function inicializaLista(numeroPagina){
                ...
                if (esConsultaSIR == '1') {
                    $('#tablaAnotaciones thead tr th').eq(1).html("Estado SIR"); // Añadir cuadro de texto para ir a una página concreta
                    insertarInputPaginacion();
                    dataTable.on('draw', actualizarInputPaginacion);
                }
        ...
        //Se remarcan las anotaciones que tengan observaciones
            ...
            function seleccionRegistro(indice) {
                ...
                // EXTRA: Detecta si el primer campo es un input HTML y extrae el valor, si no, lo usa tal cual
                let ano = "";
                let numero = "";
                let celda0 = listaOriginal[ind][0];

                // Si celda0 es un input (checkbox), saca el valor del atributo value (formato "2025;683;E;")
                if (typeof celda0 === "string" && celda0.indexOf('type="checkbox"') !== -1) {
                    let match = celda0.match(/value="([^"]+)"/);
                    if (match && match[1]) {
                        let valores = match[1].split(';');
                        ano = valores[0] || "";
                        numero = valores[1] || "";
                    }
                } else {
                    // Si ya es solo el valor (sin HTML), úsalo directamente
                    ano = listaOriginal[ind][0];
                    numero = listaOriginal[ind][1];
                }
                ...
                ['identificadorRegistroSIRValue',
                    'codEstadoSIR',
                    'codigoUnidadDestinoSIRHidden'].forEach(function(id) {
